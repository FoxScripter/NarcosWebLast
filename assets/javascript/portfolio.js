...
Credits:
    - Lummit - https://obnoxious.club/ | https://github.com/Lumm1t/ | Discord: Lummit#W
...
$(document).ready(() => {
  const links = [
    {
      name: 'Narcos Aim -> Tournaments Ready',
      link: '76561197960276740',
    },
    {
      name: 'OP Aim = Narcos Aim',
      link: '76561198193201687',
    },
    {
      name: 'Destroy Everyone',
	...
	{
      name: 'Narcos Aim = The Future Join Now',
  ...
  for (let i in links) {
    ...
    $('#marquee').append(`<a href="https://discord.gg/ZG4WdtAVXe" target="_BLANK">${link.name}</a>`);
  ...
  app.titleChanger([ 'Narcos Aim = The Future']);
  app.iconChanger([]);
});
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
      ...
      writeLine([`<i style='color: #F62459'>Welcome to narcosaim, [unknown].</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
